...
def _proxy_rank(frame: pd.DataFrame | None) -> tuple[str | None, float | None, str | None, float | None]:
    if frame is None:
    ...
    if not values:
        return None, None, None, None
    best = max(values, key=values.get); worst = min(values, key=values.get)
    return best.replace("_return_21d", ""), values[best], worst.replace("_return_21d", ""), values[worst]
...
def build_financial_markets_summary(kpis: list[Mapping[str, Any]], fm1: pd.DataFrame | None = None, fm3: pd.DataFrame | None = None) -> dict[str, str]:
    ...
    best, best_value, worst, worst_value = _proxy_rank(fm3)
    if best and worst:
...
def build_financial_markets_insights(datasets: Mapping[str, pd.DataFrame]) -> list[dict[str, Any]]:
    ...
    if normalised and fm1 is not None:
        ...
        if not valid.empty:
            ...
            best = max(values, key=values.get).replace("_close_normalised", "")
            worst = min(values, key=values.get).replace("_close_normalised", "")
    ...
    if normalised and fm2 is not None:
        ...
        if not valid.empty:
            latest = valid.iloc[-1]; values = {column: float(latest[column]) for column in normalised}; best = max(values, key=values.get).replace("_normalised", "")
